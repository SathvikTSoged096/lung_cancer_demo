...
# Expects auth_config.yaml in the same folder
try:
...
if auth_status is False:
    ...
    def overlay_heatmap_on_image(orig_rgb, heatmap, thresh=0.35):
        ...
        if orig_rgb.dtype != np.uint8:
            orig = (255 * (orig_rgb - orig_rgb.min()) / (orig_rgb.ptp() + 1e-8)).astype(
                np.uint8
            )
        else:
            orig = orig_rgb.copy()
        ...
        _, mask = cv2.threshold(
            hmap_resized, int(thresh * 255), 255, cv2.THRESH_BINARY
        )
    ...
    def run_inference_with_gradcam(model, volume_array, return_gradcam=True):
        try:
            if model is None:
                ...
                label = (
                    "ಸ್ಥೂಲ ಸಂಶಯ (Malignant)"
                    if score > 0.5
                    else "ಸಂದೇಹದಿಲ್ಲ (Benign)"
                )
                return {
                    "label": label,
                    "score": float(score),
            ...
            if return_gradcam:
                ...
                try:
                    ...
                    overlay_rgb, annotated_rgb = overlay_heatmap_on_image(
                        orig_resized[rep_idx], heatmap, thresh=0.35
                    )
                    gradcam_info = {
                        "slice_index": rep_idx,
    ...
    with col1:
        ...
        if uploaded_files:
            npy_files = [
                f for f in uploaded_files if f.name.lower().endswith(".npy")
            ]
            jpg_files = [
                f
            ...
            if len(npy_files) == 1 and len(uploaded_files) == 1:
                ...
                st.warning(
                    "Upload either a single .npy file or one/more JPG slices."
                )
        else:
                ...
                with st.spinner("Running inference + Grad-CAM..."):
                    ...
                    if "error" in result:
                        ...
                        if gc is None:
                            ...
                            st.markdown(
                                f"**Representative slice index:** {rep_idx}"
                            )
                            c1, c2 = st.columns(2)
                            with c1:
                                st.image(
    ...
    with col2:
        ...
        st.markdown(
            "Type questions in Kannada or English; the bot replies in Kannada (with speech)."
        )
        ...
        user_input = st.text_input(
            "Your question (Kannada/English):", key="user_input"
        )
        if st.button("Send"):
            if user_input.strip() == "":
